...
async fn cas_creates_updates_conflicts_and_deletes() {
    ...
    let outcome = store
        .compare_and_set(user, "fn", "k", None, &json!({"n": 1}), ROOMY)
        .await
        .unwrap();
    ...
    let outcome = store
        .compare_and_set(user, "fn", "k", None, &json!(2), ROOMY)
        .await
        .unwrap();
    assert_eq!(
        outcome,
    ...
    let outcome = store
        .compare_and_set(user, "fn", "k", Some(1), &json!({"n": 2}), ROOMY)
        .await
        .unwrap();
    assert_eq!(outcome, CasOutcome::Applied { version: 2 });
    let entry = store.get(user, "fn", "k").await.unwrap().unwrap();
    assert_eq!(entry.value, json!({"n": 2}));
    assert_eq!(entry.version, 2);

    // Update at a stale version: conflict, value untouched.
    let outcome = store
        .compare_and_set(user, "fn", "k", Some(1), &json!("stale"), ROOMY)
        .await
        .unwrap();
    assert_eq!(
        outcome,
    ...
    let outcome = store.delete(user, "fn", "k", 1).await.unwrap();
    assert!(matches!(outcome, CasOutcome::Conflict { .. }));
    let outcome = store.delete(user, "fn", "k", 2).await.unwrap();
    assert!(matches!(outcome, CasOutcome::Applied { .. }));
    assert!(store.get(user, "fn", "k").await.unwrap().is_none());

    // Updating a missing key: conflict with no current version.
    let outcome = store
        .compare_and_set(user, "fn", "gone", Some(1), &json!(1), ROOMY)
        .await
        .unwrap();
    assert_eq!(
        outcome,
...
async fn state_is_scoped_by_owner_and_function_name() {
    ...
    store
        .compare_and_set(user_a, "fn", "k", None, &json!("a"), ROOMY)
        .await
        .unwrap();
    store
        .compare_and_set(user_b, "fn", "k", None, &json!("b"), ROOMY)
        .await
        .unwrap();
    store
        .compare_and_set(user_a, "other", "k", None, &json!("c"), ROOMY)
        .await
        .unwrap();

    let a = store.get(user_a, "fn", "k").await.unwrap().unwrap();
    let b = store.get(user_b, "fn", "k").await.unwrap().unwrap();
    let c = store.get(user_a, "other", "k").await.unwrap().unwrap();
    assert_eq!(
...
async fn state_survives_revisions_and_redeploy_until_purged() {
    ...
    store
        ...
        .unwrap();
    state
        .compare_and_set(user, "sticky", "k", None, &json!("kept"), ROOMY)
        .await
        .unwrap();
    ...
    store
        ...
        .await
        .unwrap();
    assert!(state.get(user, "sticky", "k").await.unwrap().is_some());
    ...
    assert!(state.get(user, "sticky", "k").await.unwrap().is_none());
}
...
async fn limits_account_exactly_and_refusals_change_nothing() {
    ...
    store
        .compare_and_set(user, "fn", "a", None, &json!("0123456789"), tight)
        .await
        .unwrap();
    store
        .compare_and_set(user, "fn", "b", None, &json!("0123456789"), tight)
        .await
        .unwrap();

    // Third key: refused by the key limit, and nothing is written.
    let err = store
        .compare_and_set(user, "fn", "c", None, &json!(1), tight)
        .await
        .unwrap_err();
    assert!(err.contains("state limit") && err.contains("keys"), "{err}");
    assert!(store.get(user, "fn", "c").await.unwrap().is_none());

    // Growing a value past the byte budget: refused, old value intact.
    let big = json!("x".repeat(50));
    let err = store
        .compare_and_set(user, "fn", "a", Some(1), &big, tight)
        .await
        .unwrap_err();
    assert!(
    ...
    let entry = store.get(user, "fn", "a").await.unwrap().unwrap();
    ...
    store.delete(user, "fn", "b", 1).await.unwrap();
    let outcome = store
        .compare_and_set(user, "fn", "c", None, &json!(1), tight)
        .await
        .unwrap();
...
async fn simultaneous_cas_has_exactly_one_winner() {
    ...
    for i in 0..16 {
        ...
        tasks.push(tokio::spawn(async move {
            store
                .compare_and_set(user, "fn", "contested", None, &json!(i), ROOMY)
                .await
                .unwrap()
        }));
    }
    ...
    let entry = store.get(user, "fn", "contested").await.unwrap().unwrap();
...
async fn writes_invalidate_other_instances_via_the_notify_payload() {
    ...
    a.compare_and_set(user, "fn", "k", None, &json!("v1"), ROOMY)
        .await
        .unwrap();
    // Instance A caches the read.
    assert_eq!(
        a.get(user, "fn", "k").await.unwrap().unwrap().value,
        json!("v1")
    );
    ...
    b.compare_and_set(user, "fn", "k", Some(1), &json!("v2"), ROOMY)
        .await
        .unwrap();
    a.invalidate(user, "fn");
    let entry = a.get(user, "fn", "k").await.unwrap().unwrap();
...
async fn keys_and_values_are_bounded_and_lists_page_in_order() {
    ...
    assert!(store.get(user, "fn", "").await.is_err());
    assert!(store.get(user, "fn", &"k".repeat(513)).await.is_err());
    let oversized = json!("x".repeat(65 * 1024));
    assert!(store
        .compare_and_set(user, "fn", "k", None, &oversized, ROOMY)
        .await
        .is_err());

    for key in ["b/2", "a/1", "b/1", "c/1"] {
        store
            .compare_and_set(user, "fn", key, None, &json!(key), ROOMY)
            .await
            .unwrap();
    }
    let (page, cursor) = store.list(user, "fn", "b/", "", 1).await.unwrap();
    assert_eq!(page.len(), 1);
    assert_eq!(page[0].key, "b/1");
    let cursor = cursor.expect("more pages");
    let (page, cursor) = store.list(user, "fn", "b/", &cursor, 10).await.unwrap();
    assert_eq!(page.len(), 1);
    assert_eq!(page[0].key, "b/2");
    assert!(cursor.is_none());

    let (all, _) = store.list(user, "fn", "", "", 100).await.unwrap();
